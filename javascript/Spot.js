...
var Spot = function (initObj){
  ...
  this.drawSkeleton = function (){
    ...
    vectors_.forEach(function(elem, i){
      var j = i;
      var currentPoint=Geometry.getVectorPosition (vectors_[j],angle_*j,this.spot.center);

      j = (i-1) < 0 ? this.snots.n-1 : i-1 ;
      var prevPoint =Geometry.getVectorPosition (vectors_[j],angle_*j,this.spot.center);

      j = (i+1)> this.snots.n-1 ? 0 :i+1 ;
      var nextPoint=Geometry.getVectorPosition (vectors_[j],angle_*j,this.spot.center);
  ...
  this.draw = function (){
    ...
    vectors_.forEach(function(elem, i){
      var j = i;
      var currentPoint=Geometry.getVectorPosition (vectors_[j],angle_*j,this_.spot.center);

      j = (i-1) < 0 ? this_.snots.n-1 : i-1 ;
      var prevPoint = Geometry.getVectorPosition (vectors_[j],angle_*j,this_.spot.center);

      j = (i+1)> this_.snots.n-1 ? 0 :i+1 ;
      var nextPoint = Geometry.getVectorPosition (vectors_[j],angle_*j,this_.spot.center);
